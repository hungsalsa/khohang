<?php

namespace backend\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\Nhaphang;

/**
 * NhaphangSearch represents the model behind the search form about `backend\models\Nhaphang`.
 */
class NhaphangSearch extends Nhaphang
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['mahdn', 'macongty', 'manhanvien', 'soluong', 'gianhap', 'status', 'created_at', 'updated_at'], 'integer'],
            [['mahang', 'donvitinh'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Nhaphang::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'mahdn' => $this->mahdn,
            'macongty' => $this->macongty,
            'manhanvien' => $this->manhanvien,
            'soluong' => $this->soluong,
            'gianhap' => $this->gianhap,
            'status' => $this->status,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ]);

        $query->andFilterWhere(['like', 'mahang', $this->mahang])
            ->andFilterWhere(['like', 'donvitinh', $this->donvitinh]);

        return $dataProvider;
    }
}
